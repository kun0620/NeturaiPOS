/*
      # Update email_change to prevent NULL issues (Data Only)

      This migration addresses the "converting NULL to string is unsupported" error
      by ensuring that the `email_change` column in `auth.users` never contains `NULL` values.
      ...
      1. Data Migration
        - Updates all existing rows in `auth.users` where `email_change` is `NULL`
          to an empty string (`''`).
      2. Security
      ...
      For new users, ensure that `email_change` is explicitly set to a non-NULL value
